...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const magnitudeLengthA = Math.sqrt(x1 * x1 + y1 * y1);
  const magnitudeLengthB = Math.sqrt(x2 * x2 + y2 * y2);
  const angleCosine = dotProduct / (magnitudeLengthA * magnitudeLengthB);
  const angleInRadians = Math.acos(Math.max(-1, Math.min(1, angleCosine)));
...
function getIntegerOnString(str, base) {
  const parseNum = Number.parseInt(str, base);
  return parseNum.isNumber ? NaN : parseNum;
}
